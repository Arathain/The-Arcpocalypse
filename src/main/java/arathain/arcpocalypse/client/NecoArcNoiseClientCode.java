...
public class NecoArcNoiseClientCode {
    public static void packetProxy(PlayerEntity player, int lastAttackedTicks) {
        ...
        new ArcpocalypseNetworking.ArcSoundEmitterPacket(player.getUuid(), player.getPos(), (player.getAttacking() != null || player.getAttacker() != null || lastAttackedTicks < 100), player.getComponent(ArcpocalypseComponents.ARC_COMPONENT).getNecoType().textureName).write(buf);
