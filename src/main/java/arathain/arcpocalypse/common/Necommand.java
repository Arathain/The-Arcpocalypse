...
public class Necommand {
	...
	public static void registerNeco(CommandDispatcher<ServerCommandSource> dispatcher) {
		dispatcher.register((LiteralArgumentBuilder) ((LiteralArgumentBuilder) CommandManager.literal("neco").requires((source) -> source.hasPermissionLevel(2))).executes((context) -> executeNeco((ServerCommandSource)context.getSource(), ImmutableList.of(((ServerCommandSource)context.getSource()).getEntityOrThrow()), NekoArcComponent.TypeNeco.ARC))
	...
	public static void registerUnneco(CommandDispatcher<ServerCommandSource> dispatcher) {
		dispatcher.register((LiteralArgumentBuilder) ((LiteralArgumentBuilder) CommandManager.literal("deneco").requires((source) -> {
